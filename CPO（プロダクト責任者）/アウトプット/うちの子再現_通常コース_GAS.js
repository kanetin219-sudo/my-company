// うちの子再現アーティスト養成講座 通常コース（6ヶ月）管理システム
// スプレッドシートID: 10nN917-O7OV8Ig9OT0OA_LFyp1oTH6-yAnYC8-r-I1g
...
function addManagementColumns() {
  ...
  // 数式設定（ARRAYFORMULA）
  sheet.getRange('B2').setFormula('=ARRAYFORMULA(IF(A2:A="","",EDATE(A2:A,6)))');
  sheet.getRange('C2').setFormula('=ARRAYFORMULA(IF(B2:B="","",B2:B-TODAY()))');
  // 継続の期日: 銀行振込→満了日+6ヶ月、カード決済→満了日+1ヶ月
  sheet.getRange('F2').setFormula('=ARRAYFORMULA(IF(E2:E="銀行振込",EDATE(B2:B,6),IF(E2:E="カード決済",EDATE(B2:B,1),"")))');
...
function checkExpiryAndNotify() {
  ...
  if (alertMessages.length > 0) {
    var subject = '【うちの子再現 通常コース】受講期日が近い受講生がいます';
